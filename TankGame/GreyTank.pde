public class GreyTank extends Tank {
  ...
  public GreyTank (float x, float y, int id) {
    super(x, y, 0, id, color(99, 90, 81), 1, 180, 1, 0, x, y);
  }
  ...
  public void move() {
    ...
    if (shortestBulletLength < shortestBombLength && shortestBulletLength < 150) {
      ...
      if (movementCounter == 0) {
        pathfind((int)random(2), new int[]{(int)(player.y/40), (int)(player.x/40)});
        movementCounter = 200;
      } else if (movementCounter > 0) {
        angleOfMovement = atan2((newLocation[0]*40+20)-this.y, (newLocation[1]*40+20)-this.x);
      }
  ...
  public void shoot() {
    if (currentBullet < maxBullet && shootCoolDown == 0 && (predictShooting(player).equals("player")||(int)(random(500))==0)) {
      shoot.trigger();
