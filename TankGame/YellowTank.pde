public class YellowTank extends Tank {
  ...
  public YellowTank(float x, float y, int id) {
    super(x, y, 0, id, color(188, 178, 55), 1, 180, 1, 4, x, y);
  }
  ...
  public void shoot() {
    if (currentBullet < maxBullet && shootCoolDown == 0 && (predictShooting(player).equals("player")||(int)(random(2000))==0)) {
      allBullets.add(new Bullet(x+15+cos(direction)*35, y+15+sin(direction)*35, 8, direction, id, 1, maxBulletId+1, "normal"));
      maxBulletId++;
