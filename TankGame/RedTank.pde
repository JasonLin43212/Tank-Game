public class RedTank extends Tank {
  ...
  public RedTank (float x, float y, int id) {
    super(x, y, 0, id, color(204, 94, 107), 3, 70, 1, 0, x, y);
  }

  //maybe move it to Tank class
  public void move() {
    super.move();
    up=0;
    down=0;
    ...
    if (shortestBulletLength < shortestBombLength &&
      shortestBulletLength < 150 &&
      (closestBullet.parentId != id || closestBullet.currentBounce == 1)) {
      movementCounter = 0;
      ...
      shouldMove = true;
      //fill(30, 30, 30);
      //ellipse(newLocation[1]*40+20, newLocation[0]*40+20, 5, 5);
    }
    if (shouldMove) {
  ...
  public void shoot() {
    if (currentBullet < maxBullet && shootCoolDown == 0 && (predictShooting(player).equals("player")||(int)(random(10000))==0)) {
      shoot.trigger();
